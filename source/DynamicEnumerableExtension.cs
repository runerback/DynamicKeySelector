...
namespace DynamicKeySelector
{
	public static class DynamicEnumerableExtension
	{
		...
		{
			return source.GroupBy(item => new DynamicGroupKey(item, properties), DynamicGroupKey.EqualityComparer);
		}
		...
		{
			return source.GroupBy(item => new DynamicGroupKey(item, properties), elementSelector, DynamicGroupKey.EqualityComparer);
		}
	}
}
